...
class ContentResultArguments {
  final ContentResult contentResult;
  final dynamic tarotList;
  final bool checkLiked;
  final void Function()? onLikeContent;
  ContentResultArguments({
    required this.contentResult,
    required this.checkLiked,
    this.onLikeContent,
    this.tarotList,
  });
}

class ContentPurchasedResultScreen extends StatefulWidget {
  const ContentPurchasedResultScreen(
      {super.key, this.contentResultArguments, this.appContentSubmit});

  final ContentResultArguments? contentResultArguments;
  final ContentInputArgument? appContentSubmit;

  @override
  State<ContentPurchasedResultScreen> createState() =>
      _ContentPurchasedResultScreenState();
}

class _ContentPurchasedResultScreenState
    extends PageStateful<ContentPurchasedResultScreen, ContentPurchasedBloc> {
  ...
  void afterFirstBuild(BuildContext context) {
    if (widget.appContentSubmit != null) {
      bloc.initial(widget.appContentSubmit!);
      // bloc.getUserInfo(widget.appContentSubmit!.content.id);
    }
    ...
    return BlocListener<ContentPurchasedBloc, ContentPurchasedState>(
      listener: (context, state) {
        state.status.whenOrNull(
          idle: () => showLoading(context, false),
          loading: () => showLoading(context, true),
          failure: (error) {
          ...
          success: (data) {
            ...
            bloc.loadShareLink(state.contentResult!.content.id);
      ...
      child: BlocBuilder<ContentPurchasedBloc, ContentPurchasedState>(
        ...
        builder: (context, state) {
          if (state.contentResult == null) {
            return Scaffold(
              ...
              appBar: AppHeader(
                context,
                titleText: widget.appContentSubmit != null
                    ? widget.appContentSubmit?.content.name
                    : widget.contentResultArguments!.contentResult.content.name,
              ),
            );
          }
          return Scaffold(
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                                ...
                                builder: (context) {
                                  return WShareDialog(
                                    ...
                                    onShareLinkClicked: () async {
                                      var link = await DeepLinkService()
                                          .createDeeplink(
                                              bloc.state.contentResult?.purchase
                                                      .id
                                                      .toString() ??
                                                  "",
                                              bloc.state.shareLink);
                        ...
                        // AppSize.space15.verticalSpace,
                        // Text(
                        //     LocaleKeys.content_detail_result_share_suggest
                        //         .tr(args: [
                        //       result.content.name
